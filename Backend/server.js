    ...
    const usePostgres = true;
     ...
     async function ensureTradeRequestsTable() {
       ...
       if (!exists) {
         ...
         CREATE TABLE IF NOT EXISTS trade_requests (
           ...
           status VARCHAR(50) DEFAULT 'pending',
           offer_details TEXT,
           receiver_seen SMALLINT DEFAULT 0,
           sender_seen SMALLINT DEFAULT 0,
           created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
         )
       `, 'create trade_requests');
       }

       await safeSchemaExecute('ALTER TABLE trade_requests ADD COLUMN IF NOT EXISTS receiver_seen SMALLINT DEFAULT 0', 'alter trade_requests receiver_seen');
       await safeSchemaExecute('ALTER TABLE trade_requests ADD COLUMN IF NOT EXISTS sender_seen SMALLINT DEFAULT 0', 'alter trade_requests sender_seen');
     }
     ...
     app.get('/api/user/:userId/stats', async (req, res) => {
       try {
         ...
         const stats = {
           ownedCount: Number(rows[0]?.ownedCount || 0),
           totalCO2: (Number(co2Rows[0]?.totalCO2 || 0) * 0.1).toFixed(1)
         };
      ...
      app.get('/api/trade/requests/:userId', async (req, res) => {
        try {
          ...
          const [incoming] = await db.execute(`
            ...
            LEFT JOIN users u ON r.sender_id = u.id::text
            WHERE r.receiver_id = ?
          ...
          const [outgoing] = await db.execute(`
            ...
            FROM trade_requests r
            JOIN plants p ON r.plant_id::integer = p.id
            LEFT JOIN users u ON r.receiver_id = u.id::text
            WHERE r.sender_id = ?
            ORDER BY r.created_at DESC
          `, [userId]);
